    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            if isinstance(self.ant, QueenAnt):
                if not self.ant.impostor:
                    self.ant.place = insect.place
                else:
                    self.ant = None
            elif isinstance(self.ant, BodyguardAnt) and isinstance(self.ant.ant, QueenAnt):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place is not hive:
            if current_place.bees and self.min_range <= count_range and count_range <= self.max_range:
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while current_place is not None:
                if isinstance(current_place.ant, Ant) and not isinstance(current_place.ant, QueenAnt):
                    current_ant = current_place.ant
                    if current_ant not in self.ants and not isinstance(current_ant, BodyguardAnt):
                        current_ant.damage *= 2
                        self.ants.append(current_ant)
                    elif isinstance(current_ant, BodyguardAnt):
                        if current_ant.ant and current_ant.ant not in self.ants:
                            current_ant.ant.damage *= 2
                            self.ants.append(current_ant.ant)
                        if current_ant not in self.ants:
                            current_ant.damage *= 2
                            self.ants.append(current_ant)
